...
def check_daily_summary(state):
    ...
    close_dt = datetime.fromtimestamp(effective_close, timezone.utc).replace(tzinfo=None)
...
def cleanup_old_keys(state):
    ...
    if state.get("today_open_ts", 0):
        ...
        ts_date = datetime.fromtimestamp(state["today_open_ts"], timezone.utc).strftime("%Y-%m-%d")
        if ts_date != today:
